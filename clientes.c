...
// Ejemplo de mejora visual en altaCliente, adaptar igual para las demás:

void altaCliente() {
    limpiarPantalla();
    printMarco();
    colorTitulo(); printCenter(3, "ALTA DE CLIENTE"); colorNormal();
    beepMenu();

    stCliente c;
    FILE* f = fopen(ARCH_CLIENTES, "ab+");
    if (!f) {
        colorError(); printCenter(7, "Error al abrir archivo de clientes."); colorNormal(); beepError(); esperaTecla(); return;
    }
    c.id = proximoId(ARCH_CLIENTES, sizeof(stCliente));
    c.nroCliente = c.id * 100 + rand() % 100;

    gotoxy(12, 7); colorDato(); printf("Nombre: "); colorNormal(); fgets(c.nombre, 30, stdin); c.nombre[strcspn(c.nombre, "\n")] = 0;
    gotoxy(12, 8); colorDato(); printf("Apellido: "); colorNormal(); fgets(c.apellido, 30, stdin); c.apellido[strcspn(c.apellido, "\n")] = 0;

    int dniExiste = 0;
    do {
        gotoxy(12, 9); colorDato(); printf("DNI: "); colorNormal(); fgets(c.dni, 10, stdin); c.dni[strcspn(c.dni, "\n")] = 0;
        if (!esNumerico(c.dni)) { colorError(); printCenter(15, "El DNI debe ser numerico."); colorNormal(); beepError(); continue; }
        rewind(f); stCliente aux; dniExiste = 0;
        while (fread(&aux, sizeof(stCliente), 1, f)) {
            if (aux.eliminado == 0 && strcmp(aux.dni, c.dni) == 0) {
                colorError(); printCenter(15, "Ya existe un cliente con ese DNI."); colorNormal(); beepError(); dniExiste = 1; break;
            }
        }
    } while(dniExiste);

    gotoxy(12,10); colorDato(); printf("Email: "); colorNormal(); fgets(c.email, 30, stdin); c.email[strcspn(c.email, "\n")] = 0;
    gotoxy(12,11); colorDato(); printf("Calle: "); colorNormal(); fgets(c.domicilio.calle, 30, stdin); c.domicilio.calle[strcspn(c.domicilio.calle, "\n")] = 0;
    gotoxy(12,12); colorDato(); printf("Nro: "); colorNormal(); fgets(c.domicilio.nro, 6, stdin); c.domicilio.nro[strcspn(c.domicilio.nro, "\n")] = 0;
    gotoxy(12,13); colorDato(); printf("Localidad: "); colorNormal(); fgets(c.domicilio.localidad, 50, stdin); c.domicilio.localidad[strcspn(c.domicilio.localidad, "\n")] = 0;
    gotoxy(12,14); colorDato(); printf("Provincia: "); colorNormal(); fgets(c.domicilio.provincia, 40, stdin); c.domicilio.provincia[strcspn(c.domicilio.provincia, "\n")] = 0;
    gotoxy(12,15); colorDato(); printf("CP: "); colorNormal(); fgets(c.domicilio.cpos, 6, stdin); c.domicilio.cpos[strcspn(c.domicilio.cpos, "\n")] = 0;
    gotoxy(12,16); colorDato(); printf("Telefono Fijo: "); colorNormal(); fgets(c.telefonoFijo, 12, stdin); c.telefonoFijo[strcspn(c.telefonoFijo, "\n")] = 0;
    gotoxy(12,17); colorDato(); printf("Telefono Movil: "); colorNormal(); fgets(c.telefonoMovil, 12, stdin); c.telefonoMovil[strcspn(c.telefonoMovil, "\n")] = 0;
    c.eliminado = 0;

    fwrite(&c, sizeof(stCliente), 1, f);
    fclose(f);
    colorMenu(); printCenter(20, "\nCliente dado de alta correctamente."); colorNormal(); beepConfirm();
    esperaTecla();
}
...
void listarClientes() {
    ...
    if(!hay) printf("No hay clientes activos.\n");
    fclose(f);
    esperaTecla();
}
